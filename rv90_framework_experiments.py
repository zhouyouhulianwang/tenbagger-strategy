#!/usr/bin/env python3
"""RV90-as-gate + alternative framework combo (v8.8 base, full pool).

  rv90_full    : MAX_VOLATILITY=0.90 (replaces 60% gate) + VIX-halve
                 + trail21 stop + RV90 buy-block + breadth scaling
  rv90_no_trail: same but WITHOUT the 21d-high -15% stop (worst layer last round)
Writes results to data/rv90_framework_experiments.json
"""
import os, sys, json
import numpy as np
import pandas as pd
...
OUT = os.path.join(DATA, 'rv90_framework_experiments.json')


def mk(trail21):
    c = Config()
    c.MAX_VOLATILITY = 0.90
    c.VIX_HALVE_ENABLED = True
...
VARIANTS = [('rv90_full', mk(True)), ('rv90_no_trail', mk(False))]
