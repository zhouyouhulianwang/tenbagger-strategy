#!/usr/bin/env python3
"""Volatility gate threshold experiments (v8.8 base, full pool).

MAX_VOLATILITY gates momentum entry AND the global volatility filter.
  gate_60 (base) : 0.60
  gate_90        : 0.90  (user's RV90 proposal)
  gate_off       : 9.99  (effectively disabled - reference)
Writes results to data/vol_gate_experiments.json
"""
import os, sys, json
import numpy as np
import pandas as pd
...
OUT = os.path.join(DATA, 'vol_gate_experiments.json')

VARIANTS = [('gate_60', 0.60), ('gate_90', 0.90), ('gate_off', 9.99)]


def run_one(gate, prices, benchmark, vix, fundamentals):
    cfg = Config()
    cfg.MAX_VOLATILITY = gate
...
def main():
    ...
    for name, gate in VARIANTS:
        ...
        results[name] = run_one(gate, prices, benchmark, vix, fundamentals)
        r = results[name]
