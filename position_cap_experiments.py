#!/usr/bin/env python3
"""Single-position cap sensitivity (v8.7 base, full pool, 2020-01 -> latest).

Single-bit config mutations of MAX_SINGLE_POSITION_PCT (current 0.30):
  cap_30 (base) / cap_25 / cap_20 / cap_15
Writes results to data/position_cap_experiments.json
"""
import os, sys, json
import numpy as np
import pandas as pd
...
OUT = os.path.join(DATA, 'position_cap_experiments.json')

VARIANTS = [('cap_30', 0.30), ('cap_25', 0.25), ('cap_20', 0.20), ('cap_15', 0.15)]


def run_one(cap, prices, benchmark, vix, fundamentals):
    cfg = Config()
    cfg.MAX_SINGLE_POSITION_PCT = cap
    ...
    # single-name concentration stats: max daily single-position share of equity
    return {
...
def main():
    ...
    for name, cap in VARIANTS:
        ...
        results[name] = run_one(cap, prices, benchmark, vix, fundamentals)
        r = results[name]
